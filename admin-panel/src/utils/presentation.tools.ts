...
export async function presentConfirmation(message: string, header = ''): Promise<boolean> {
...
export async function presentAlert(message: string, header = ''): Promise<void> {
...
export async function presentLoading(translationKey = '') {
    ...
    const l = await loadingController.create(translationKey ? { message: await translate(translationKey) } : {})
    l.present().then()
    return l
}
